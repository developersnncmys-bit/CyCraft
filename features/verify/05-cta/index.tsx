'use client';
/* Verify CTA — pinned cinematic close. Mirrors AboutFinalCta and
 * ResearchCta composition: centered heading, ambient beam glow, dual CTA.
 * No terminal lines; this is a marketing close, not a system message.
 *
 * 200% pin. Beats (0–1):
  ...
  useGSAP(
    () => {
      ...
      if (!isDesktop) {
        ...
        gsap.fromTo(
          '.verify-cta-primary, .verify-cta-secondary',
          { opacity: 0, y: 16 },
      ...
      gsap.set('.verify-cta-word', { opacity: 0, yPercent: 70, filter: 'blur(10px)' });
      gsap.set('.verify-cta-desc', { opacity: 0, y: 16 });
      gsap.set('.verify-cta-primary', { opacity: 0, y: 18 });
      gsap.set('.verify-cta-secondary', { opacity: 0, y: 18 });
      gsap.set('.verify-cta-scan', { scaleX: 0, transformOrigin: 'left center' });
      gsap.set('.verify-cta-glow', { opacity: 0.25, scale: 1 });
      gsap.set('.verify-cta-camera', { scale: 1, transformOrigin: 'center center' });

      const tl = makePinnedTimeline({
        trigger: root,
        end: PIN_DURATIONS.verifyCta,
        scrub: 1,
        enabled: true,
      });

      tl.to('.verify-cta-scan', { scaleX: 1, duration: 0.20, ease: 'power3.out' }, 0.02);
      tl.to(
        '.verify-cta-word',
        { opacity: 1, yPercent: 0, filter: 'blur(0px)', duration: 0.25, stagger: 0.04, ease: 'power3.out' },
        0.00,
      );
      tl.to('.verify-cta-desc', { opacity: 1, y: 0, duration: 0.15, ease: 'power2.out' }, 0.30);
      tl.to('.verify-cta-primary', { opacity: 1, y: 0, duration: 0.15, ease: 'power3.out' }, 0.45);
      tl.to('.verify-cta-secondary', { opacity: 1, y: 0, duration: 0.15, ease: 'power3.out' }, 0.55);
      tl.to(
        '.verify-cta-glow',
        { opacity: 0.6, scale: 1.2, duration: 0.20, ease: 'power2.inOut' },
        0.72,
      );
      tl.to('.verify-cta-camera', { scale: 0.96, duration: 0.08, ease: 'power2.inOut' }, 0.92);
    },
    { scope: sectionRef, dependencies: [isDesktop, reducedMotion] },
  );

  const prefixWords = verifyCtaContent.headlinePrefix.trim().split(/\s+/).filter(Boolean);
  const accentWords = verifyCtaContent.headlineAccent.trim().split(/\s+/).filter(Boolean);

  return (
    <section
      ref={sectionRef}
      id="verify-cta"
      aria-label={`${verifyCtaContent.headlinePrefix} ${verifyCtaContent.headlineAccent}`}
      style={{
        position: 'relative',
        minHeight: '100vh',
        overflow: 'hidden',
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <Link
              href={verifyCtaContent.primaryCta.href}
              className="verify-cta-primary"
              style={{
                display: 'inline-flex',
                alignItems: 'center',
                justifyContent: 'center',
            ...
            >
              {verifyCtaContent.primaryCta.label}
