'use client';
/* Verify Hero — autoplay reveal on load (NOT pinned).
 *
 * Mirrors AboutHero / ResearchHero composition: centered, autoplay
 * word-by-word entry after the Preloader finishes. The h1 sits on a
 * single line ("Certificate Verification") with the accent in beam-cyan.
 *
 * Light scroll parallax (scrubbed, not pinned) drifts the layers as the
 * next pinned act (Form) engages.
 */
...
import { verifyHeroContent } from '@/content/verify/hero';
...
export default function VerifyHero() {
  ...
  useGSAP(
    () => {
      ...
      const revealTargets = [
        '.verify-hero-badge',
        '.verify-hero-line-1 [data-word]',
        '.verify-hero-line-2 [data-word]',
        '.verify-hero-divider',
        '.verify-hero-tagline [data-word]',
        '.verify-hero-pill',
        '.verify-hero-terminal-line',
      ];
      ...
      gsap.set(['.verify-hero-badge', '.verify-hero-terminal-line'], { opacity: 0, y: 18 });
      gsap.set('.verify-hero-line-1 [data-word], .verify-hero-line-2 [data-word]', {
        opacity: 0,
        yPercent: 60,
        filter: 'blur(10px)',
      });
      gsap.set('.verify-hero-divider', { scaleX: 0, transformOrigin: 'left center' });
      gsap.set('.verify-hero-tagline [data-word]', { opacity: 0, y: 8, filter: 'blur(4px)' });
      gsap.set('.verify-hero-pill', { opacity: 0, y: 10 });

      const playEntry = () => {
        const tl = gsap.timeline({ defaults: { ease: 'power3.out' } });
        tl.to('.verify-hero-badge', { opacity: 1, y: 0, duration: 0.6 })
          .to(
            '.verify-hero-line-1 [data-word]',
          ...
          .to('.verify-hero-divider', { scaleX: 1, duration: 0.5 }, '-=0.25')
          .to(
            '.verify-hero-tagline [data-word]',
            { opacity: 1, y: 0, filter: 'blur(0px)', duration: 0.5, stagger: 0.022 },
            '-=0.2',
          )
          .to('.verify-hero-pill', { opacity: 1, y: 0, duration: 0.4, stagger: 0.08 }, '-=0.3')
          .to(
            '.verify-hero-terminal-line',
      ...
      gsap.to('.verify-hero-bg-layer', { yPercent: 12, ease: 'none', scrollTrigger: parallax });
      gsap.to('.verify-hero-mid-near', {
        yPercent: 25,
        scale: 1.15,
        opacity: 0.45,
        ease: 'none',
  ...
  return (
    ...
    >
      <div
        aria-hidden="true"
        className="verify-hero-bg-layer"
        style={{
          position: 'absolute',
          inset: '-10%',
          zIndex: 1,
      ...
      <div
        aria-hidden="true"
        className="verify-hero-mid-near"
        style={{
          position: 'absolute',
          top: '50%',
          left: '50%',
      ...
      <div
        className="verify-hero-fg section-container"
        style={{
          position: 'relative',
          zIndex: 6,
          width: '100%',
      ...
      >
        <div style={{ maxWidth: '960px', marginInline: 'auto' }}>
          ...
          >
            ...
            <Badge label={verifyHeroContent.badge} />
          </div>
          ...
          >
            <WordSplit
              className="verify-hero-line-1"
              text={verifyHeroContent.headlinePrefix}
              style={{
                display: 'inline',
            ...
            />{' '}
            <WordSplit
              className="verify-hero-line-2"
              text={verifyHeroContent.headlineAccent}
              style={{
                display: 'inline',
          ...
          >
            <WordSplit text={verifyHeroContent.tagline} />
          </p>
          ...
          >
            {verifyHeroContent.pills.map((pill) => (
              <span
                key={pill.label}
                className="verify-hero-pill"
                style={{
                  display: 'inline-flex',
                  alignItems: 'center',
                  gap: '0.5rem',
          ...
          >
            {verifyHeroContent.terminalLines.map((line) => (
              <span key={line} className="verify-hero-terminal-line">
                {line}
              </span>
            ))}
          </div>
